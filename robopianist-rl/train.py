...
def save_agent_checkpoint(path: Path, agent: sac.SAC, step: int, args: "Args") -> None:
    """Save SAC params and training Args for offline Q(s,a) / ΔQ evaluation."""
    ...
    blob = {
        ...
        "temp": jax.device_get(agent.temp.params),
    }
...
def main(args: Args) -> None:
    ...
    for i in tqdm(range(1, args.max_steps + 1), disable=not args.tqdm_bar):
        ...
        if i % args.eval_interval == 0:
            ...
            try:
                vf = eval_env.latest_filename
                if vf is not None and Path(vf).is_file():
                    video = wandb.Video(str(vf), fps=4, format="mp4")
                    wandb.log({"video": video, "global_step": i})
            except (OSError, ValueError, FileNotFoundError, RuntimeError) as e:
                print(f"[train] Skipping eval video log: {e}")
            save_agent_checkpoint(experiment_dir / f"checkpoint_{i}.pkl", agent, i, args)
            save_agent_checkpoint(experiment_dir / "checkpoint_latest.pkl", agent, i, args)
